...
class LeCartWoocommerce(LeCartWordpress):
	...
	def check_product_import(self, convert, product, products_ext):
		...
		meta_insert = {
			# 'meta_value': self.strip_html_tag(get_value_by_key_in_dict(convert, 'qty', '')),
			'meta_value': 'a:1:{s:9:"testattri";a:6:{s:4:"name";s:9:"TestAttri";s:5:"value";s:4:"test";s:8:"position";i:0;s:10:"is_visible";i:1;s:12:"is_variation";i:0;s:11:"is_taxonomy";i:0;}}'
		}
		where_meta = {
			'post_id': product_import,
			'meta_key': '_product_attributes'
		}
		self.import_product_data_connector(self.create_update_query_connector('postmeta', meta_insert, where_meta), True,
										   convert['id'])
	...
	def product_import(self, convert, product, products_ext):
		...
		product_data = {
			...
			'post_parent': 0,
			'guid': self._notice['target']['cart_url'] + "/?product=" + code_product[:200],
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		# is_subscribed = php_unserialize(customer['meta_value'])
		if 'subscriber' in php_unserialize(customer['meta_value']):
			customer_data['is_subscribed'] = True
